...
const PaymentOffices = () => {
  ...
  const getAllPaymentOffices = async () => {
    const response = await getAllThirdParties();
    if (response) {
      const paymentOffices = response.filter((item) => item.Type === 1);
      setData(paymentOffices);
    }
    setLoading(false);
  };

  React.useEffect(() => {
    getAllPaymentOffices();
  ...
  return (
    ...
    >
      ...
      <TableBody
        emptyContent={"No hay pagadurías encontradas"}
        items={sortedItems}
        isLoading={loading}
